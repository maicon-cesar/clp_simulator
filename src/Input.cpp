...
void Input::SetKeyboardBind(sf::Keyboard::Key key, Action action)
{
	...
	{
		fprintf(stderr, "erro em SetKeyboardBind\n");
	}
}
...
void Input::SetJoystickBind(unsigned int joybutton, Action action)
{
	...
	{
		fprintf(stderr, "error em SetJoystickBind\n");
	}
}
...
unsigned int Input::GetJoystickBind(Action action)
{
	...
	if (action < Input::COUNT)
		return action_to_joybutton_[action];
	else 
		return MAX_JOY_BUTTON;	
}


//const std::wstring& Input::ActionToString(Action action)
const sf::String Input::ActionToString(Action action)
{;
	switch (action) {
		case PAUSE:
			return "pause";
...
void Input::AskUserInput(Device device, Action action)
{
	...
	prompt.setPosition(
		(int) (Application::WIDTH - rect.width) / 2,
		(int) (Application::HEIGHT - rect.height) / 2
	);

	sf::Event event;
	bool running = true;
	sf::RenderWindow& app = Application::GetInstance().GetApp();
	while (running)
	{
